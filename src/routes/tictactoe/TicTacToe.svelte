<script lang="ts">
	...
	function getWinner(): string {
		for (let combination of getCombinations()) if (allEqual(combination)) return combination[0];
		if (board.every((column) => noneEmpty(column))) return 'tie';
		return '';
	}
	...
	function getCombinations(): string[][] {
		const rows = Array.from({ length: size }, (_, row) =>
			Array.from({ length: size }, (_, column) => board[column][row])
		);
		const columns = Array.from({ length: size }, (_, column) =>
			Array.from({ length: size }, (_, row) => board[column][row])
		);
...
<div class="main">
	...
	<div class="board" style="--size: {size * size}rem">
		{#each board as column, columnIdx (columnIdx)}
			<div class="column">
				{#each column as field, rowIdx (rowIdx)}
					<button class="field" onclick={() => chooseField(columnIdx, rowIdx)}
						><span class={board[columnIdx][rowIdx]}> {field}</span></button
					>
				{/each}
			</div>
		{/each}
